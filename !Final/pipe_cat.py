...
def feat_gen(df) -> pd.DataFrame: # Генерация признаков
    ...
    pre_loans_columns = ['pre_loans5_#CAT#_', 'pre_loans530_#CAT#_', 'pre_loans3060_#CAT#_', 'pre_loans6090_#CAT#_', 'pre_loans90_#CAT#_']
    df['count_active_loan_#CAT#_'] = df[pre_loans_columns].sum(axis=1)

    # Процент погашения задолженности от общей суммы кредита
    df['perc_debt_repayment'] = df.apply(
        lambda x: x['pre_loans_outstanding_#CAT#_'] / x['pre_loans_credit_limit_#CAT#_'] if x['pre_loans_credit_limit_#CAT#_'] > 0 else 0, axis=1)

    # Периодичность просрочек
    df['diff_pre_loans_#CAT#_'] = df[pre_loans_columns].max(axis=1) - df[pre_loans_columns].min(axis=1)
    ...
    # Сумма переплаты
    df['Overpayment_amount_#CAT#_'] = df['pre_loans_credit_cost_rate_#CAT#_'] - df['pre_loans_credit_limit_#CAT#_']
    ...
    # флаги просрочек
    is_zero_loans = [
        "is_zero_loans5_#CAT#_",
        "is_zero_loans530_#CAT#_",
        "is_zero_loans3060_#CAT#_",
        "is_zero_loans6090_#CAT#_",
        "is_zero_loans90_#CAT#_"
    ]
    df[is_zero_loans] = df[is_zero_loans].replace([0, 1], [1, 0])

    df["loan_overdue_#CAT#_"] = df[is_zero_loans].any(axis=1)

    # веса для просрочек
    def set_weights(x):
        return x['pre_loans5_#CAT#_'] + x['pre_loans530_#CAT#_'] * 2 + x['pre_loans3060_#CAT#_'] * 3 + x['pre_loans6090_#CAT#_'] * 4 + x['pre_loans90_#CAT#_'] * 5

    df['weights_#CAT#_'] = df.apply(set_weights, axis=1)
    q25 = df['weights_#CAT#_'].quantile(0.25)
    q75 = df['weights_#CAT#_'].quantile(0.75)
    df['cat_late_payment_#CAT#_'] = df['weights_#CAT#_'].apply(lambda x: 0.0 if x < q25 else (1.0 if x > q75 else 0.5))
...
def select_features_(X,y) -> pd.DataFrame: #Выбор признаков имеющих важность
    ...
    selected_features = summary['selected_features_names'] + ['count_active_loan_#CAT#_', 'perc_debt_repayment', 'diff_pre_loans_#CAT#_', 
                                                              'avg_stat_pay', 'Overpayment_amount_#CAT#_', 'use_loans_credit_limit', 
                                                              'loan_overdue_#CAT#_', 'weights_#CAT#_''cat_late_payment_#CAT#_']
...
def main():
    ...
    y = df['flag']
    X= X.rename(columns=lambda x: x + '_#CAT#_')
    ...
    cat_features = X.filter(like='_#CAT#_').columns.tolist()

    # Удаление из списка cat_features всех элементов, которых нет в файле selected_features.txt
